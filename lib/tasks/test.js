...
module.exports = function(options) {
	...
	var karmaConfig = options.pathPrefix + 'karma.conf.js';
	...
	gulp.task(taskPrefix + 'test:unit', [taskPrefix + 'soy'], function(done) {
		runKarma(karmaConfig, {}, function() {
			done();
		});
	});

	gulp.task(taskPrefix + 'test:coverage', [taskPrefix + 'soy'], function(done) {
		runKarma(karmaConfig, {configFile: path.resolve(options.pathPrefix + 'karma-coverage.conf.js')}, function() {
			done();
		});
	});

	gulp.task(taskPrefix + 'test:coverage:open', [taskPrefix + 'test:coverage'], function(done) {
		openFile(path.resolve(options.pathPrefix + 'coverage/lcov/lcov-report/index.html'));
		done();
	});
...
function runKarma(karmaConfig, config, done) {
	config = merge({
		configFile: path.resolve(karmaConfig),
		singleRun: true
	}, config);
